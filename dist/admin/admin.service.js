...
let AdminService = class AdminService {
    constructor(prisma, adminRealtimeGateway) {
        ...
        this.invoiceTemplateStore = new Map([
            [
                'default-ug',
                {
                    id: 'default-ug',
        ...
        this.policyStore = new Map();
        this.verticalPolicyStore = new Map([
            [
                'ride',
                {
                    verticalId: 'ride',
    ...
    async createAgent(actorId, input, meta) {
        const now = Date.now();
        const agent = {
            id: (0, uuid_1.v4)(),
            name: input.name,
    ...
    async patchAgent(actorId, agentId, patch, meta) {
    ...
    async deleteAgent(actorId, agentId, meta) {
        const existing = await this.getAgent(agentId);
    ...
    async patchTaxConfig(actorId, regionId, patch, meta) {
        const existing = this.taxConfigStore.get(regionId) ??
            {
                regionId,
                currency: 'UGX',
                vatPercent: 18,
    ...
    async patchInvoiceTemplate(actorId, templateId, patch, meta) {
        const existing = this.invoiceTemplateStore.get(templateId) ??
            {
                id: templateId,
                regionId: 'UG',
    ...
    async createTrainingModule(actorId, input, meta) {
        const now = Date.now();
        const module = {
            id: (0, uuid_1.v4)(),
            title: input.title,
    ...
    async patchTrainingModule(actorId, moduleId, patch, meta) {
    ...
    async deleteTrainingModule(actorId, moduleId, meta) {
        const existing = this.trainingModuleStore.get(moduleId);
        if (!existing) {
            throw new common_1.NotFoundException('Training module not found');
        }
    ...
    async createPolicy(actorId, input, meta) {
        const now = Date.now();
        const policy = {
            id: (0, uuid_1.v4)(),
            key: input.key,
    ...
    async patchPolicy(actorId, policyId, patch, meta) {
    ...
    async patchVerticalPolicy(actorId, verticalId, patch, meta) {
        const existing = this.verticalPolicyStore.get(verticalId) ??
            {
                verticalId,
