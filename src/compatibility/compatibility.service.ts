...
export class CompatibilityContractService {
  ...
  private readonly canonicalContracts: AppCanonicalContract[] = [
    {
      appId: 'rider',
      rest: {
  ...
  getCanonicalContract(appId: AppId): AppCanonicalContract {
    ...
    if (!contract) {
      throw new NotFoundException(`Canonical contract not found for app: ${appId}`);
    }
    return contract;
  }
